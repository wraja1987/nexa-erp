...
# Resolve repo root relative to this script (supports being sourced) unless REPORTS_DIR is provided by environment
...
audit_after_phase(){
  ...
  local t="$(ts_slug)"
  local safe_phase="${phase_name//\//_}"
  safe_phase="${safe_phase// /_}"
  local LOG="$REPORTS_DIR/phase-${phase_no}-${safe_phase}-${t}.log"
  local MD="$REPORTS_DIR/phase-${phase_no}-${safe_phase}-${t}.md"
