...
export function redisLimiter({ windowMs, max, keyPrefix = "rl" }: LimiterOpts) {
  ...
  return async (key: string): Promise<Verdict> => {
    ...
    if (!client) {
      // No redis available at runtime; fall back seamlessly
      return inMemoryLimiter({ windowMs, max, keyPrefix })(key);
    }
    const k = `${keyPrefix}:${key}`;
    // Compatible with ioredis and node-redis v5
    const count = Number(await (client as any).incr(k));
    let ttl = Number(await (client as any).ttl(k));
    if (ttl < 0) {
      await (client as any).expire(k, Math.ceil(windowMs / 1000));
