...
import { getShortTermCashflow } from "@/server/banking/cash";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const to = searchParams.get("to") ? new Date(String(searchParams.get("to"))) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    ...
    const data = await getShortTermCashflow(scope, from, to);
