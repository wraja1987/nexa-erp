...
import { previewOpeningBalancesImport, applyOpeningBalancesImport } from "@/server/imports/openingBalances";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Preview first
    const preview = await previewOpeningBalancesImport({ tenantId, userId }, csv);
    if (!preview.supported) {
      return Response.json({ ok: false, error: preview.message || "Not supported" }, { status: 501 });
    }

    // Apply
    const result = await applyOpeningBalancesImport({ tenantId, userId }, preview.rows);
