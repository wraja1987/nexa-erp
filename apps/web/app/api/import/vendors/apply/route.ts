...
import { previewVendorImport, applyVendorImport } from "@/server/imports/masterData";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Preview first
    const preview = await previewVendorImport({ tenantId, userId }, csv);
    if (!preview.supported) {
      return Response.json({ ok: false, error: preview.message || "Not supported" }, { status: 501 });
    }

    // Apply
    const result = await applyVendorImport({ tenantId, userId }, preview.rows);
