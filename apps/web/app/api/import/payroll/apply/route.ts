...
import { previewPayrollImport, applyPayrollImport } from "@/server/imports/payroll";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Preview first
    const preview = await previewPayrollImport({ tenantId, userId }, csv);
    if (!preview.supported) {
      return Response.json({ ok: false, error: preview.message || "Not supported" }, { status: 501 });
    }

    // Apply
    const result = await applyPayrollImport({ tenantId, userId }, preview.rows);
