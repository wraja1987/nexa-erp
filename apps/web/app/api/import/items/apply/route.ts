...
import { previewItemImport, applyItemImport } from "@/server/imports/masterData";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Preview first
    const preview = await previewItemImport({ tenantId, userId }, csv);
    if (!preview.supported) {
      return Response.json({ ok: false, error: preview.message || "Not supported" }, { status: 501 });
    }

    // Apply
    const result = await applyItemImport({ tenantId, userId }, preview.rows);
