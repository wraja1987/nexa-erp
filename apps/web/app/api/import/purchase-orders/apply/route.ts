...
import { previewPurchaseOrderImport, applyPurchaseOrderImport } from "@/server/imports/orders";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Preview first
    const preview = await previewPurchaseOrderImport({ tenantId, userId }, csv);
    if (!preview.supported) {
      return Response.json({ ok: false, error: preview.message || "Not supported" }, { status: 501 });
    }

    // Apply
    const result = await applyPurchaseOrderImport({ tenantId, userId }, preview.rows);
