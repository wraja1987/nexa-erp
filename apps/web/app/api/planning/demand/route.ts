...
import { getDemandPlan } from "@/server/planning/service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getDemandPlan(tenantId, {
      horizonMonths,
      bucketSize,
      itemId,
      warehouseId,
    ...
    if (!result.supported) {
      return Response.json(
        { ok: false, supported: false, error: result.reason || "Demand planning not supported" },
        { status: 501 }
      );
    }
    ...
    captureError(error, { module: "planning", operation: "get_demand" });
