...
import { getSupplyPlan } from "@/server/planning/service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getSupplyPlan(tenantId, {
      horizonMonths,
      bucketSize,
      itemId,
      warehouseId,
    ...
    if (!result.supported) {
      return Response.json(
        { ok: false, supported: false, error: result.reason || "Supply planning not supported" },
        { status: 501 }
      );
    }
    ...
    captureError(error, { module: "planning", operation: "get_supply" });
