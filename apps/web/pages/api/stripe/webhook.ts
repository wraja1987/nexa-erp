...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  } catch { /* continue; rely on DB upserts */ }

  const idemKey = (req.headers["idempotency-key"] as string) || "";
  if (idemKey) {
    const idem = await ensureIdempotent(`wh:${idemKey}`, 300);
    if (idem === "duplicate") { res.status(202).json({ ok: true, deduped: true }); return; }
  }

  try {
    switch (event.type) {
      case "payment_intent.succeeded":
        // TODO: upsert payment by external_id
        break;
      case "charge.refunded":
        // TODO: upsert refund by external_id
        break;
      default: break;
    }
