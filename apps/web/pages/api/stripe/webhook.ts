...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // De-dup by event.id (24h TTL)
  const r = getRedis();
  const k = "wh:" + stripeEventKey(event.id);
  const already = await r.set(k, "1", "EX", 60*60*24, "NX");
  if (!already) {
    auditLog({ type: "stripe_webhook_dupe", eventId: event.id });
    res.status(200).json({ ok: true, duplicate: true });
    return;
  }

  // Optional: also respect Idempotency-Key for any database write we perform from this webhook
  const idemKey = (req.headers["idempotency-key"] as string) || "";
  if (idemKey) {
    const idem = await ensureIdempotent(`wh:${idemKey}`, 300);
    if (idem === "duplicate") {
  ...
  try {
    ...
    switch (event.type) {
      case "payment_intent.succeeded":
        // TODO: write payment record; ensure your DAL is naturally idempotent (UPSERT on unique external_id)
        break;
      case "charge.refunded":
        // TODO: write refund record
        break;
      default:
        break;
    }
