...
function Kpis({ items = [] as any[] }) {
  ...
  return (
    <section className="col-span-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {items.map((k: any) => (
        <div key={k.id} className="rounded-2xl border border-slate-200 bg-white shadow-sm p-4">
          <div className="text-slate-600 text-xs">{k.label}</div>
              ...
              {it.actions?.length ? (
                <div className="mt-2">
                  {it.actions.map((a: any, i: number) => (
                    <a key={i} href={a.href} className="text-sky-700 hover:underline text-sm">{a.label}</a>
                  ))}
                </div>
              ) : null}
            </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="px-4 py-2">
          <ul className="space-y-1">
            {data.items.map((it: any) => (
              <li key={it.id}><a href={it.href} className="block rounded-lg px-3 py-2 hover:bg-slate-50 text-sm text-slate-700">{it.label}</a></li>
            ))}
          </ul>
        </div>
      </div>
      ...
      <div className="px-4 py-3 overflow-x-auto" role="region" aria-label={data.title || "Data table"}>
        <table className="w-full text-sm">
          <thead className="text-slate-600">
            <tr>
              {cols.map((c: string, i: number) => (<th key={i} className="text-left py-2 pr-4">{c}</th>))}
            </tr>
          </thead>
          <tbody>
...
function AiEngine({ data }: { data: any }) {
  ...
  return (
    <div className="p-4">
      <div className="text-sm font-medium text-slate-900 mb-2">{data.title || "AI Engine"}</div>
      <div className="flex gap-2">
        <input
          placeholder={data.placeholder || "Ask…"}
          aria-label={data.placeholder || "Ask AI"}
          className="h-10 flex-1 rounded-xl border border-slate-200 px-3 text-sm outline-none focus:ring-2 focus:ring-sky-400"
        />
        <button className="h-10 rounded-xl bg-sky-600 text-white px-4 text-sm hover:bg-sky-700">Send</button>
      </div>
    </div>
  );
}
