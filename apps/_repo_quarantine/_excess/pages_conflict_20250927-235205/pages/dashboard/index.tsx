...
type DashboardData = { cards?: Card[]; links?: LinkItem[]; insights?: string[] };
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 12 }}>
        {cards.map((c) => (
          <div key={c.id} style={{ background: "#fff", border: "1px solid #E5EAF1", borderRadius: 12, padding: 16, boxShadow: "0 4px 16px rgba(15,39,71,0.06)" }}>
            <div style={{ fontSize: 12, opacity: 0.7 }}>{c.title}</div>
            <div style={{ fontSize: 24, fontWeight: 700, marginTop: 2 }}>{c.value ?? "—"}</div>
            {c.hint ? <div style={{ fontSize: 12, color: "#506481", marginTop: 4 }}>{c.hint}</div> : null}
          </div>
        ))}
      </section>
      ...
      {links.length > 0 && (
        <section>
          <h2 style={{ margin: "8px 0" }}>Quick Actions</h2>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {links.map((l) => (
              <a key={l.href} href={l.href} style={{ padding: "8px 12px", background: "#2E6BFF", color: "#fff", borderRadius: 8, textDecoration: "none" }}>
                {l.label}
              </a>
            ))}
          </div>
...
export const getServerSideProps: GetServerSideProps<{ data: DashboardData }> = async (ctx) => {
  ...
  try {
    ...
    data = {
      cards: [
        { id: "welcome", title: "Welcome", value: "Signed in", hint: "Demo data" },
        { id: "revenue", title: "Revenue", value: "£0", hint: "—" }
      ],
      links: [{ label: "Finance", href: "/modules/finance" }],
      insights: ["AI is monitoring your KPIs", "No anomalies detected in the last 24h"]
    };
  }
