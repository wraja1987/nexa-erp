...
BEGIN
  ...
  -- SUPER: use email as id if inserting
  INSERT INTO public."User"(id, email, password_hash, role, active, created_at)
  VALUES (current_setting('app.super_email',true), current_setting('app.super_email',true), current_setting('app.super_hash',true), 'SUPER_ADMIN', true, now())
  ON CONFLICT (email) DO UPDATE SET password_hash = EXCLUDED.password_hash, role = 'SUPER_ADMIN', active = true;

  -- ADMIN: id = email, bound to tenant
